...
static void pkvm_test_debug_ctrl(struct kunit *test)
{
	u64 msr_val;
	u64 msr_val_to_restore;

	rdmsrq(MSR_IA32_DEBUGCTLMSR, msr_val);
	msr_val_to_restore = msr_val;
	if (!msr_val) {
		// Enable bit 15 Bus Lock Detect
		msr_val |= DEBUGCTLMSR_RTM_DEBUG;
		wrmsrq(MSR_IA32_DEBUGCTLMSR, msr_val);
	}

	KUNIT_ASSERT_EQ_MSG(test, pkvm_hypercall(test, VM_EXIT_ENTRY_CTRLS,
			MSR_IA32_DEBUGCTLMSR, msr_val),
			    0, "pkvm_test_debug_ctrl: failed\n");
